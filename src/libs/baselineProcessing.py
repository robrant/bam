...
def upsertBaselineDoc(dbh, doc, outputs, collection='baseline'):
    ...
    q = {'mgrs'          : doc['mgrs'],
         'keyword'       : doc['keyword'],
         'mgrsPrecision' : doc['mgrsPrecision']}
...
def getResultsPerCell(dbh, collection, mgrs, keyword, inDate, lookback=None, countOnly=None):
    ...
    if countOnly:
        res = dbh[collection].find(q).count()
    else:
        res = dbh[collection].find(q)
...
class baseline(object):
    ...
    def __init__(self, mgrs, mgrsPrecision, keyword, timeStamp, c=None, dbh=None, host=None, port=None, db=None, baselinePrecision=[]):
        ''' Constructor. '''
